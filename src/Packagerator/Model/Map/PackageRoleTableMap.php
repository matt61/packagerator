...
use Packagerator\Model\PackageRole;
use Packagerator\Model\PackageRoleQuery;
...
/**
 * This class defines the structure of the 'package_role' table.
 *
 *
 *
 ...
 *
 */
class PackageRoleTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Packagerator.Model.Map.PackageRoleTableMap';
     ...
     */
    const TABLE_NAME = 'package_role';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\Packagerator\\Model\\PackageRole';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Packagerator.Model.PackageRole';
     ...
     */
    const COL_PACKAGE_ID = 'package_role.package_id';
    ...
    {
        // attributes
        $this->setName('package_role');
        $this->setPhpName('PackageRole');
  ...
  array (
    ...
    {
        return $withPrefix ? PackageRoleTableMap::CLASS_DEFAULT : PackageRoleTableMap::OM_CLASS;
    }
     ...
     * @return array           (PackageRole object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PackageRoleTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PackageRoleTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PackageRoleTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PackageRoleTableMap::OM_CLASS;
            /** @var PackageRole $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PackageRoleTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PackageRoleTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PackageRoleTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PackageRoleTableMap::DATABASE_NAME)->getTable(PackageRoleTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PackageRoleTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PackageRoleTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PackageRoleTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a PackageRole or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or PackageRole object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Packagerator\Model\PackageRole) { // it's a model object
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ((array) $values as $singleval) {
                PackageRoleTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PackageRoleQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a PackageRole or Criteria object.
     *
     * @param mixed               $criteria Criteria or PackageRole object containing data that is used to create the INSERT statement.
...
//
PackageRoleTableMap::buildTableMap();
